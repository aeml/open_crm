...
for the largest lazy chunk, and 645.38 KiB/206.97 KiB total assets. Hosted
...
- `1.1.9` (shared team inbox foundation): complete and privacy-hardened, but not promoted beyond foundation. Shared inbound queue metadata, assignment/open/closed coordination, and member-safe detail access remain. Sharing a private synced message now presents an explicit complete-body disclosure confirmation, while the mailbox owner can make it private again with immediate access removal and team metadata cleanup. The service—not a stale handler pre-read—revalidates the active writer, tenant, mailbox-owner/admin privacy authority, and active writer assignee inside the row-locking transaction; an exact response version turns concurrent close/assignment/privacy actions into a reloadable `409` instead of last-writer-wins. Each real transition commits one content-free audit event, while identical replay is a no-op. Handler/UI and disposable-PostgreSQL tests cover reversible privacy, disabled/read-only/foreign assignees, cross-tenant denial, stale and simultaneous updates, idempotency, and audit counts. Threaded reply UX and the product policy for sending as a mailbox owner remain incomplete; one-to-one tracking now has a local consent/retention contract, while approved live-provider/pilot evidence remains open.
