...
CSS. Current evidence is 178.03 KiB/57.86 KiB for the entry, 47.00 KiB/12.23 KiB
for the largest lazy chunk, and 649.89 KiB/205.98 KiB total assets. Hosted
billing remains an isolated 7.30 KiB/2.66 KiB route and retry-key creation is a
0.15 KiB shared helper; the aggregate raw ceiling is now effectively full, so
the next frontend slice must offset new code. Tested route
...
- `1.0.9` (trial-expiry / cancellation enforcement): partial. The shared `CheckWritable` decision treats canceled, expired-trial, unpaid, paused, incomplete, and incomplete-expired state as read-only while active, in-period trial, and past-due payment-retry grace remain writable. A centralized route-derived boundary now applies that decision after authentication and role checks to every registered private tenant mutation, including update/delete/provider actions and the credential-writing OAuth callback; an executable registration scan proves coverage. It fails closed with `503` when entitlement state cannot be read, while reads, CSV exports, billing recovery, profile/preferences, and notification acknowledgement remain available. Public lead capture locks and checks the destination subscription plus contact limit, returning a leak-safe unavailable response. Every non-billing tenant worker checks the same policy before effects and uses a durable 15-minute deferral that does not consume attempts; reconciliation remains exempt so recovery continues. Backend and disposable-PostgreSQL tests cover the decision, full route set, role ordering, recovery exceptions, public submission, deferral, and signed provider transitions. Remaining: persistent read-only/suspended UI navigation and controls, explicit dunning timing/retention, and offboarding.
...
- `1.0.3`/`1.0.4` (foundation): complete. Added migration `014_billing_plans.sql` (`plan` column on `organizations` with a CHECK constraint); a `billing` module with an in-code plan catalog (Free/Starter/Pro/Enterprise: seat/contact/deal limits + feature keys) and an entitlements service computing live usage; `GET /api/billing/plans` and `GET /api/billing/entitlements`; a centralized private-write/public-lead/worker lifecycle boundary; and a "Plan & Billing" settings page showing usage, hosted state, and plan comparison. Remaining: persistent read-only UI state, plan-feature enforcement, and complete usage metering; SSO remains `1.0.7`.
