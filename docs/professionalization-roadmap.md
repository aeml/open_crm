...
- `1.1.5` (templates/snippets/merge fields): complete locally; approved live-provider/pilot evidence pending. The authenticated merge catalog combines core contact/company/deal tokens with active tenant contact/company custom fields under collision-safe `contact.custom.*` and `company.custom.*` namespaces. Known missing custom values render blank; unknown tokens remain visible, are returned as a distinct sorted preview list, and block every provider send. Settings manages organization-scoped templates/snippets and exposes the dynamic catalog. Contact/company/deal composers use the same server render for exact recipient/subject/body preview and the durable private test-to-self boundary described in `1.1.6`. Handler/unit/component/migration tests, disposable-PostgreSQL exact-recipient/private/no-customer-timeline acceptance, and the real-SMTP browser custom-field preview/test/WCAG journey cover the local job. Remaining: approved SMTP/Gmail/Microsoft test evidence and pilot validation of vocabulary, blank values, safety notice, and recovery wording.
