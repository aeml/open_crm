...
- `1.0.4` (limit enforcement): complete for current static hosted resources. In Stripe mode, `EnforceCanCreate` and `CanCreateMore` cover contacts, deals, and seats; authenticated creates reject over-limit writes with `402 PLAN_LIMIT_REACHED`, and public lead-created contacts check the same plan without exposing tenant state. Configured enforcement fails closed with `503` when usage cannot be verified. Fake/self-hosted mode returns unlimited local usage and bypasses these hosted ceilings across authenticated and public paths. Frontend write forms surface managed limit errors and the billing UI labels the self-hosted boundary. Backend, frontend, and disposable-PostgreSQL tests cover managed block/allow/failure plus self-hosted private/public bypass. Durable billing-period meters and concurrency-safe quotas for messages/jobs/API/storage remain `1.0.5` work.
- `1.0.5` (explainable source-reconciled usage): partial. Migration `077_billing_usage_snapshots.sql` persists the exact Stripe period start, adds one retained observation per tenant/period, and adds bounded partial indexes for period sources. A member-scoped API and Plan & Billing view reconcile active seats/contacts/deals, sent outbound email, successful workflow runs and durable jobs within the provider period or UTC-month fallback, plus estimated current row bytes across tenant-scoped PostgreSQL base tables. Reconciliation uses one repeatable-read transaction, dynamically quotes a bounded source list, upserts the period snapshot, exposes source/unit/scope/basis/observation details, and never makes the measurement itself an unapproved quota. PostgreSQL tests cover boundaries, repeatability, fallback, retained update, and foreign-tenant exclusion. Remaining: approve feature/tier/limit policy; design concurrency-safe quota reservation/enforcement across UI, API, and workers; add API-use metering only when a versioned external API exists; decide index and external-object storage accounting.
