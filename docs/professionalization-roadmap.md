...
- `1.1.2` (automatic generic IMAP sync worker): complete. Added due-account discovery for enabled generic IMAP/password mailboxes and an in-process mailbox sync worker that starts after API boot, polls due accounts every 15 minutes, runs the same ingestion path as `POST /api/me/email-sync/run`, and logs batch import/failure counts. Gmail OAuth accounts are now included by the same worker; Microsoft Graph and a durable external job runner remain future slices.
