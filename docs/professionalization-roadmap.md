...
CSS. Current evidence is 178.78 KiB/58.13 KiB for the entry, 46.96 KiB/12.20 KiB
for the largest lazy chunk, and 649.81 KiB/205.81 KiB total assets. Hosted
billing remains an isolated 7.31 KiB/2.65 KiB route and retry-key creation is a
0.15 KiB shared helper; centralized workspace-role decisions plus compact root
navigation/profile metadata offset the persistent access-state UI without
raising the effectively full aggregate ceiling. Tested route
splits plus bulk/custom-field/touchpoint/close-review/account/health integration leave contacts
at 1,299 lines, companies at 984, deals at 1,064, and tasks at 839, down from 2,038,
...
- `1.0.2` (Stripe-hosted lifecycle foundation): partial. Migration `074_stripe_billing_lifecycle.sql` adds tenant-bound durable Checkout attempts, provider references and ordering cursors, signed-event receipts, and an invoice ledger. Stripe mode creates backend-hosted Checkout and customer-portal sessions with pinned API version, bounded timeouts/responses, tenant metadata, and provider idempotency; direct browser/API plan activation is rejected. The public webhook verifies the raw-body HMAC within a five-minute replay window, rejects test/live mismatches, deduplicates event IDs and payloads, safely retries failed receipts, fails cross-tenant references closed, and applies ordered subscription/invoice state plus audit/provider telemetry. Signed subscription events—not return URLs—activate plans and reconcile provider trials, dunning grace, payment recovery, suspension, scheduled cancellation, and cancellation. Migration `075_billing_reconciliation.sql` adds tenant reconciliation attempt/success/error evidence and a bounded due index. Stripe workspaces are discovered every 15 minutes when six-hour evidence is stale, then a leased durable job retrieves current subscription state and the recent 25 invoices. It validates every tenant/customer/subscription reference, uses a retrieval-start watermark so provider polling cannot overwrite newer webhooks, records audit/provider/failure evidence, retries transient errors, and exposes dead jobs for labeled admin replay in Operations. Provider/UI tests and disposable-PostgreSQL lifecycle/reconciliation acceptance cover schedule deduplication, ordering, invoice refresh, durable retry, and cross-tenant failure. Remaining before promotion: approved Stripe test-mode purchase/portal/cancel/retry/reconciliation acceptance, explicit portal/proration/resubscription policy, automated dunning timing, feature/period metering, and tenant export/deletion.
- `1.0.9` (trial-expiry / cancellation enforcement): partial. The shared `CheckWritable` decision treats canceled, expired-trial, unpaid, paused, incomplete, and incomplete-expired state as read-only while active, in-period trial, and past-due payment-retry grace remain writable. A centralized route-derived boundary applies that decision after authentication and role checks to every registered private tenant mutation, including update/delete/provider actions and the credential-writing OAuth callback; an executable registration scan proves coverage. It fails closed with `503` when entitlement state cannot be read, while reads, CSV exports, billing recovery, profile/preferences, and notification acknowledgement remain available. Public lead capture locks and checks the destination subscription plus contact limit, returning a leak-safe unavailable response. Every non-billing tenant worker checks the same policy before effects and uses a durable 15-minute deferral that does not consume attempts; reconciliation remains exempt so recovery continues. Login, email verification, and current-session responses now include a server-derived access snapshot without making authentication depend on billing availability. Shared client state shows a persistent read-only/unavailable banner, immediately reacts to `402`/`503` policy responses, and consistently removes or disables write/admin actions across CRM, inbox, configuration, import, restore, and saved-view surfaces while retaining navigation, reads, exports, billing, profile/preferences, notifications, and job replay. Backend, frontend, and disposable-PostgreSQL tests cover the decision, full route set, role ordering, recovery exceptions, public submission, deferral, signed provider transitions, and read-only journey. Remaining: explicit dunning timing/retention and offboarding.
- `1.0.6`/`1.0.9` (subscription status + trial): partial. Migration `015_subscription_lifecycle.sql` introduced bounded subscription states and trial end; migration `073_verified_workspace_signup.sql` adds an auditable local trial start. New self-serve workspaces remain inaccessible with no running trial clock until the owner consumes the one-time email link, when verification atomically starts the 14-day period and first session. Entitlements expose local or Stripe-reconciled status, trial/current-period end, cancellation schedule, and provider state; the billing page shows that state. Fake-provider changes activate locally, while Stripe mode changes only after a valid subscription webhook. Backend and disposable-PostgreSQL tests cover pending, active, expired, grace, suspended, recovery, and canceled timing. Remaining: approved provider acceptance, explicit proration/resubscription behavior, broad suspension enforcement, retention, and deletion.
...
- `1.0.4` (limit enforcement): complete for current static resources. `EnforceCanCreate` and `CanCreateMore` cover contacts, deals, and seats; authenticated creates reject over-limit writes with `402 PLAN_LIMIT_REACHED`, and public lead-created contacts check the same plan without exposing tenant state. Configured enforcement fails closed with `503` when usage cannot be verified; a nil service remains compatible with isolated/self-hosted callers. Frontend write forms surface the API message. Backend and disposable-PostgreSQL tests cover decision, block/allow/failure, and public recovery paths. Durable billing-period meters and concurrency-safe quotas for messages/jobs/API/storage remain `1.0.5` work.
- `1.0.2` (provider seam + fake default): complete. Added a `billing.Provider` interface with a `FakeProvider` (no external calls; default for tests and self-hosted/development deployments), fail-closed unconfigured providers, and bounded provider telemetry. `BILLING_PROVIDER` selects the provider. `POST /api/billing/change-plan` and the direct plan-switch UI remain fake-only; Stripe mode requires the hosted flow described above. `example.env` documents the safe default and exact hosted configuration boundary. Remaining work belongs to the partial Stripe lifecycle outcome rather than the provider seam.
- `1.0.3`/`1.0.4` (foundation): complete. Added migration `014_billing_plans.sql` (`plan` column on `organizations` with a CHECK constraint); a `billing` module with an in-code plan catalog (Free/Starter/Pro/Enterprise: seat/contact/deal limits + feature keys) and an entitlements service computing live usage; `GET /api/billing/plans` and `GET /api/billing/entitlements`; a centralized private-write/public-lead/worker lifecycle boundary; session-derived persistent read-only UI state; and a "Plan & Billing" settings page showing usage, hosted state, and plan comparison. Remaining: plan-feature enforcement and complete usage metering; SSO remains `1.0.7`.
