...
- `1.1.1` (real delivery via Postmark): complete. Added a `PostmarkProvider` (mirrors the Mendola customer-panel sender) selected by `EMAIL_PROVIDER=postmark` with `POSTMARK_SERVER_TOKEN` / `POSTMARK_FROM_EMAIL` / `POSTMARK_MESSAGE_STREAM`. `NewProvider` is now config-based (fake / postmark / unconfigured-stub). An unconfigured Postmark provider fails loudly rather than silently no-opping. `example.env` documents Postmark as the active provider. Tests cover provider selection, configuration, and send guards (no network).
