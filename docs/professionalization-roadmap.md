...
500-line CI ceiling. All 271 explicit registrations live in focused 184-line
platform, 312-line foundation, and 380-line core-CRM files, called centrally by
...
producers retain the same policy under the reviewed source-set digest. These
identity domains are now independently reviewable. The former 487-line
email-message handler is likewise split into 224-line authenticated message and
shared-inbox operations, 221-line response projection/access decisions, and a
56-line public tracking boundary. Rejected or unsafe click destinations now
receive the same no-store, no-referrer, and no-index headers as successful
redirects. Every production file in `internal/app` is
...
work, touchpoint, production outreach, and 166-line create/detail
...
at 441 lines, down from 2,038 and below the default 500-line ceiling. Tested 68-line selection and
...
epochs; the shared 362-line record-email composer owns server-rendered preview, private test-to-self, direct-send, and recovery state. A tested 59-line lead-score hook remains a development/API seam and rejects duplicate
in-flight evaluations, mismatched contact identities, and late responses after
leave-and-return navigation. Production contact orchestration and bundles omit manual scoring execution while a focused component retains read-only historical score evidence. Development-only call, SMS, and meeting orchestration remains
...
activities, quote lines, and quote/signature/approval evidence on the active deal; the guarded
416-line commercial hook shares that contract while a 284-line quote-version
component owns the retained evidence and review controls. The generic follower control
...
A 111-line shared definition-catalog boundary now owns abortable latest-request
identity, exact-page search/status loading, page-one search reconciliation,
mutation exclusion, filters, and pagination used by product, quote-template,
email-sequence, and email-template/snippet administration. The four larger route files are
218/310/266/267 lines; together with the 228-line lead-surface boundary and its
118/126-line routes, it keeps the original definition-catalog reduction. A
23-line controlled field in the existing UI chunk then removes another 131
production source lines across definition, lead-surface, contact, client, deal,
task, quote, record-work, mailbox, workflow, and reporting forms. The combined
measured build lowers aggregate raw assets from 816.13 to 807.89 KiB and gzip
from 254.51 to 254.33 KiB without changing the 817/255 KiB ceilings; the largest
...
- Added `docs/tenant-isolation-matrix.md` and an executable digest guard, now at 35 rows, that binds every promoted service family to an exact freshly migrated PostgreSQL acceptance test. Missing files/tests, non-PostgreSQL substitutions, count changes, and undocumented evidence changes fail CI.
...
- Role and status writes now revalidate the acting owner/admin inside the serializable tenant transaction. A role transition commits its previous/current-role audit event atomically, retries bounded serialization/deadlock failures, creates no duplicate for an unchanged request, and rolls back the role if audit insertion fails. The current member row and complete role/lifecycle history are both included in the portable workspace export and the browser journey verifies the filtered audit CSV.
...
Current convergence evidence: all 109 registered GET routes are digest-gated to
...
Chromium search/link/primary/unlink/WCAG journey cover the outcome. Shared inbox
now defaults to 50 and caps at 100 with a strict snapshot-bound opaque cursor
over open/closed bucket, effective received time, and ID. Accessible continuation
deduplicates by ID and retains the newest optimistic version; freshly migrated
...
WCAG scan. Quote-template management now uses the same 50/default,
100/maximum, and 50,000-offset shape with literal name search, active/archived
filters, exact repeatable-read totals, and active/name/ID order. A tenant-scoped
writer lock revalidates the active owner/admin and serializes the 100-active
ceiling across API instances. Deal preparation loads only active pages,
deduplicates IDs, rejects changing totals, and continues for legacy overflow.
Fresh 1,001-row PostgreSQL acceptance proves indexed active selection,
adjacent pages below two seconds, literal wildcard search, tenant/role denial,
the final-slot race, and archive/reactivation recovery. Chromium loads/searches
row 51, creates the pilot template, and carries it through finalization,
approval, delivery, signing, and the populated WCAG scan. Email-sequence
definition management now applies the same bounded trust boundary: exact
repeatable-read 50/default and 100/maximum management pages,
literal name/status filters, a 50,000 offset ceiling, selected-page outcome
aggregation, service-level actor revalidation, exact-revision update/delete/
approval, and transactional lifecycle audit. A tenant advisory lock serializes
the 100-active ceiling, while enrollment/nurture selectors load every active
page, reject total drift, and retain legacy overflow. Fresh 1,001-definition
PostgreSQL acceptance proves the indexed pages, two-second budgets, tenant/role
...
The 400-line route is 13.61/4.30 KiB raw/gzip, and the current production-URL
Node 24 build is 179.11/58.09 KiB entry, 54.10/15.65 KiB largest lazy chunk,
and 809.27/255.07 KiB aggregate raw/gzip. Only the aggregate gzip ceiling
...
- Added an hourly, multi-instance-safe retention pass: successful payload/result detail compacts after 30 days and successful idempotency rows expire after 400 days in bounded `SKIP LOCKED` batches. Active and dead jobs are excluded, only eleven explicitly reviewed production job types are eligible (including exact-artifact scheduled-report delivery), current producers retain source-state duplicate guards, and PostgreSQL acceptance covers both cutoffs, unknown-type preservation, tenant-wide operation, idempotence, and batch limits.
...
row contact export under a 5 s budget, runs a tenant-isolated 500-deal pipeline
...
custom line/funnel/pie/KPI controls and omits booking, audience/scoring management and manual evaluation/routing,
...
and focused contact outreach/historical-score/workspace/detail orchestration plus shared record selection/work, company directory/people/workspace/detail orchestration, and task directory/workspace presentation leave contacts at 441 lines,
...
- `1.1.6` (durable one-to-one record-email boundary): complete locally; approved live-provider evidence pending. Migrations `106_record_email_deliveries.sql` and `107_record_email_template_tests.sql` route contact/company/deal customer sends and explicit test-to-self deliveries through one actor-scoped idempotent state machine. The API persists the exact rendered sender/recipient/content/RFC/tracking snapshot before SMTP/Gmail/Microsoft, validates the exact active tenant customer or acting user/sign-in address, claims once under concurrency, and rechecks active membership, connected sender identity, and the purpose-specific recipient immediately before the effect. Customer sends additionally recheck suppression and atomically commit email/link/note/activity/audit evidence. Tests force tracking/suppression/unsubscribe/HTML rewriting off, prefix a safety notice, and retain only a private unlinked email plus separate audit evidence—never a customer link/note/activity. A browser retains the same purpose-bound key across transport ambiguity and resets it when composition changes. Definite failures retain purpose-correct failed email evidence; ambiguous SMTP/OAuth effects and five-minute interrupted sends become visible `uncertain` records and are never automatically retried. Only the original active sender may explicitly retry after a duplicate-risk warning and only while hosted writes are active; owners/admins may confirm sent or mark not sent without a provider call during suspension. Deactivation quiesces prepared/claimed record emails and threaded replies, startup/minutely recovery is observable through aggregate alerts, normal product navigation labels unresolved tests separately, terminal intents drop duplicate tracking material without extending the 90-day window, and portable export removes hashes/RFC/provider/HTML/tracking/signed-link material. Handler/component/metrics/migration/export tests, freshly migrated PostgreSQL replay/conflict/exact-customer/exact-user/private-test/no-timeline/tenant/permission/concurrent-claim/atomicity/failure/recovery/lifecycle acceptance, and the real-SMTP Chromium preview/test/customer-send plus forced post-acceptance disconnect/reload/no-resend confirmation with WCAG cover the local boundary. Remaining: retain approved SMTP/Gmail/Microsoft ambiguity recovery evidence and validate the preview/test/recovery wording with a pilot.
...
- `1.1.4` (click tracking foundation): privacy/retention contract complete locally; provider/pilot validation pending. Random 256-bit tokens and server-stored validated HTTP(S) targets remain behind separate fail-closed shared public limits. Successful redirects and every rejected, missing, provider-failed, or unsafe target receive the same no-store/no-referrer/no-index response boundary. The same explicit 90-day window gates collection. At expiry and after observation scrubbing, old delivered links continue to redirect without incrementing message or link counters; target mappings remain only to preserve that recipient-facing behavior. UI copy labels signals approximate, exposes off/active/expired state truthfully, and a focused policy document records consent responsibility and retained data. Real mail-scanner/privacy-proxy behavior and pilot policy wording remain validation work.
- `1.1.5` (templates/snippets/merge fields): complete locally; approved live-provider/pilot evidence pending. The authenticated merge catalog combines core contact/company/deal tokens with active tenant contact/company custom fields under collision-safe `contact.custom.*` and `company.custom.*` namespaces. Known missing custom values render blank; unknown tokens remain visible, are returned as a distinct sorted preview list, and block every provider send. Settings gives templates and snippets independent exact searchable 50/default and 100/maximum pages with exact totals, stale-load and cross-catalog mutation guards, exact-revision update/delete, transactional active-writer revalidation and content-free lifecycle audit, plus separate serialized 100-stored-definition ceilings. Record composers load the complete stable bounded catalogs, preserve legacy overflow, and use the same server render for exact recipient/subject/body preview and the durable private test-to-self boundary described in `1.1.6`. Handler/library/component/migration tests, freshly migrated 1,001-row PostgreSQL paging/index/tenant/actor/revision/audit/final-slot/recovery acceptance, disposable-PostgreSQL exact-recipient/private/no-customer-timeline acceptance, and the real-SMTP browser template/snippet row-51/search/custom-field preview/test/WCAG journey cover the local job. Remaining: validate both operating ceilings plus vocabulary, blank values, safety notice, and recovery wording with a pilot, and retain approved SMTP/Gmail/Microsoft test and ambiguity evidence.
...
- `1.1.8` (production-equivalent sequence delivery acceptance): complete locally; capability promoted to `production-capable`, with real-provider/pilot validation still pending. The clean PostgreSQL Chromium pilot journey now drafts a one-step cadence, proves admin approval of its exact revision, enrolls a real contact through the record UI, waits for the durable job worker to deliver exactly once through the real SMTP adapter and local provider sandbox, inspects the merged recipient, multipart body, prepared RFC `Message-ID`, and unsubscribe fallback in captured MIME, verifies the tenant aggregate reaches one provider acceptance and one finished cadence, and retains a WCAG A/AA scan. The promoted tenant evidence set now names the PostgreSQL runner test that rejects a cross-tenant job and proves exact-once delivery plus explicit ambiguous-outcome recovery. The same journey found and now guards a long client name/URL pointer-overlap regression through bounded record-link wrapping. Remaining: the named Gmail/Microsoft/SMTP, feedback, one-click/DKIM, deliverability, provisional-cap, and pilot evidence in the capability matrix.
- `1.1.8` (bounded sequence enrollment history): complete. Migration `114_email_sequence_enrollment_history.sql` adds the tenant/sequence/creation/ID access path for an opaque newest-first cursor that defaults to 50 and caps at 100. Enrollment creation now revalidates the active same-tenant enroller inside its transaction; direct missing/foreign actors leave no history or job, and historical actor display joins through tenant membership. The existing enrollment `GET` requires exactly one contact or sequence selector, rejects malformed/ignored pagination, and keeps sequence and organization bound through selection, detail, and per-enrollment delivery aggregation. Every member can drill from sequence totals into the contact, enroller, timestamps, terminal reason, accepted/bounce/complaint/suppressed/queued/review evidence, and the existing uncertain-delivery recovery screen. The UI loads only when expanded, deduplicates continuation rows, aborts/discards closed or stale requests, and preserves contact enrollment behavior. Handler and focused UI tests cover selectors, bounds, continuation, deduplication, and stale close/reopen; freshly migrated PostgreSQL acceptance covers active/foreign actors, 1,001 equal-time rows, repeat/adjacent/final pages, exact index use, direct-call limits, detailed outcomes, and a foreign sequence; the real-SMTP Chromium journey reconciles its aggregate against the individual finished enrollment before WCAG scanning. The 160-line route-local drill-down produces a measured 10.00/3.16 KiB route and 742.74/231.58 KiB aggregate build; reviewed aggregate ceilings advance only from 738/231 to 743/232 KiB while entry, per-route, CSS, and source ceilings remain unchanged.
- `1.1.8` (bounded sequence definition management): complete locally; operating-limit and pilot validation remain. Management now returns exact repeatable-read searchable/status-filtered pages with 50/default, 100/maximum, and 50,000-offset bounds while aggregating steps and execution outcomes only for selected definitions. Update, delete, and approval bind to the positive revision the operator reviewed; create/update/delete/approval/effective-pause revalidate the active actor and write content-free audit evidence in the same transaction, while repeated approval/pause is effect-idempotent. A tenant advisory lock serializes the 100-active ceiling across API instances. Contact/nurture selectors load only active pages, ID-deduplicate them, detect changing totals, and preserve legacy overflow. Guarded UI search/status/previous/next navigation serializes mutations and discards obsolete loads. Handler/unit/UI tests, freshly migrated 1,001-definition PostgreSQL index/performance/literal-search/revision/audit/role/tenant/final-slot acceptance, the 38-row isolation guard, and the clean Chromium row-51/search/create/approve/active-only-select/SMTP-delivery/WCAG journey cover the outcome. The measured route is 12.91/3.98 KiB and 313 lines; the complete build is 178.82/57.97 KiB entry, 54.93/15.64 KiB largest lazy chunk, and 751.69/234.60 KiB aggregate raw/gzip under reviewed 752/235 KiB aggregate ceilings. Entry, per-route, CSS, and source ceilings remain unchanged.
...
- `1.1.9` (shared team inbox and threaded replies): complete locally, but not promoted beyond foundation pending approved live-provider and pilot evidence. Shared inbound assignment/open/closed coordination retains reversible explicit-disclosure privacy, optimistic concurrency, transactional actor/assignee/tenant revalidation, immediate access removal, and content-free audit evidence. The team queue defaults to 50 and caps at 100 with a strict snapshot-bound opaque continuation over open/closed bucket, effective received time, and ID; refresh starts a new snapshot while later arrivals or coordination changes cannot duplicate into older pages. Migration `110_shared_inbox_cursor.sql` adds the matching partial index. Accessible UI continuation deduplicates IDs and serializes refresh, continuation, and coordination actions so stale loads or duplicate mutations cannot overlap. Migration `092_email_threaded_replies.sql` adds a tenant-scoped stable conversation root plus a durable request-key/hash reply ledger. Mailbox and Team Inbox render chronological threads and reply only through the acting teammate's own connected mailbox—never by impersonating the original receiver. Source privacy is inherited, RFC `Message-ID`/`In-Reply-To`/`References` and provider threads are preserved, and reply tracking is off. The provider boundary persists immutable content/header/sender evidence before one claim, rechecks sender identity and suppression immediately before effect, atomically finalizes provider acceptance with an outbound thread message, and refuses automatic replay. OAuth/SMTP/finalization ambiguity and five-minute interrupted claims become operator-visible `uncertain`; only the original actor can retry, while owners/admins can confirm sent or mark not sent, with audited resolution. An immediate/minutely bounded `SKIP LOCKED` recovery pass, aggregate gauges, validated alerts, and a runbook cover stuck/uncertain work. Handler/MIME/UI/metrics/migration tests, disposable-PostgreSQL 1,001-row continuation plus tenant/role/idempotency/claim/finalize/recovery/correlation acceptance, and Chromium continuation/WCAG evidence cover the local result. Remaining: approved Gmail/Microsoft/SMTP raw-header/thread, suppression, forced-ambiguity recovery, and pilot policy evidence.
...
- `1.1.5` Email templates, snippets, merge preview, and private test-to-self: complete locally; approved live-provider/pilot evidence remains.
...
- `1.2.5` (calendar meeting foundation): complete. Added organization-scoped calendar events, a complete 28-block-per-user availability replacement set, a fake/default calendar provider seam, APIs for listing/scheduling/cancelling meetings and current-user availability, contact-detail meeting scheduling/history/cancellation UI, and meeting activity timeline entries. Availability writes now atomically replace the set under active-writer/tenant locks with five-second deadlines, exact duplicate rejection, stable errors, and fresh-PostgreSQL permission/tenant/timeout-rollback evidence. Real Google/Microsoft OAuth calendar event sync, free/busy imports, external attendee invites, webhooks, and conflict resolution remain future provider-specific work.
- `1.2.6` (booking links foundation): complete. Added organization-scoped calendar booking links with slugs, duration/buffer/timezone metadata, active/inactive state, selected host members, owner vs round-robin assignment mode, authenticated APIs for listing/creating/updating links, and a development-only Settings > Booking Links UI for managing links and weekly availability. The complete catalog now discloses 100 links per workspace and 20 active same-tenant hosts per link; transactional active-writer/host revalidation, tenant-lock final-slot serialization, five-second deadlines, stable capacity errors, Unicode field bounds, UI edit-at-capacity recovery, and fresh-PostgreSQL role/tenant/concurrency/update/timeout evidence harden it. Production navigation/bundles deliberately omit that management route because public booking pages, guest self-scheduling, slot generation, real round-robin assignment, external calendar conflict checks, reminders, and rescheduling/cancellation flows remain future slices.
...
- `1.3.7` (multi-currency exchange-rate foundation): complete. Added organization base currency settings, manual organization exchange-rate records, admin API/UI for saving rates, and base-currency conversion for deal-list, dashboard pipeline, quota, and weighted forecast rollups while preserving per-record deal/catalog currencies. Quote disclosure is completed by `1.3.12`; automated FX providers, realized gain/loss accounting, and broader accounting outcomes remain out of the current CRM convergence scope unless pilot evidence requires them.
- `1.3.8` (immutable quote-version convergence): complete for finalization, connected-mailbox delivery, customer access, and explicit receipt. A writer can finalize saved line items with recipient, validity, and terms; one transaction locks the deal, allocates a version, snapshots all commercial identity/line/total fields, stores the exact PDF bytes and SHA-256, and records activity/audit evidence. Hashed actor-scoped idempotency collapses concurrent retries and rejects changed requests. Delivery persists an exact sender/recipient/message intent, stable RFC Message-ID, digest-only expiring customer token, and suppression check before the SMTP/Gmail/Microsoft boundary; provider acceptance records the shared outbound email/activity/audit transactionally, while ambiguous effects are quarantined for sender/admin Sent-folder resolution and never automatically retried. Preparation and claims lock and revalidate active sender membership first; user disable/revocation cannot race a late intent, fails prepared intent, and quarantines already claimed sends as uncertain in the same lifecycle transaction. Public preview/PDF access is bounded/no-store, counters are explicitly approximate, and idempotent receipt evidence is explicitly not approval or signature. Startup/minutely recovery, aggregate metrics/alerts, portable business evidence without tokens/correlation hashes, handler/UI/migration/disposable-PostgreSQL recovery/expiry/tenant tests, and the PostgreSQL browser journey through a real SMTP sandbox plus customer WCAG scan cover the boundary. Native signing continues in `1.3.9`; reusable templates, approval, and approved live-mailbox/pilot evidence remain future slices. FX disclosure is completed by `1.3.12`.
- `1.3.9` (immutable native quote-signing ceremony): complete for the first-party production-equivalent path; capability maturity remains foundation pending the rest of the Phase 4 outcome. A signature request is created only as part of delivering one immutable quote and is constrained to its tenant/deal/quote/recipient/delivery. Mailbox-provider acceptance activates the ceremony; definite failure, confirmed-not-sent recovery, or pre-send member deactivation voids it, while ambiguous delivery remains operator-resolvable without duplicate effects. A digest-only bearer link requires exact normalized recipient name, explicit immutable consent, and a digest-only 16–200 character idempotency key. Exact terminal replay returns one effect; changed reuse, new-key terminal mutation, expiry, wrong name, staff-forged completion, and cross-tenant certificate access fail closed. A successful signature atomically retains typed/expected identity, consent/time, recipient-link authentication, quote PDF SHA-256, and exact certificate PDF/SHA; public and staff downloads return identical bytes. Historical manual rows are read-only non-evidence. Shared abuse budgets, aggregate awaiting/expired/terminal gauges and alert, member-lifecycle recovery, portable certificate/consent export without replay hashes, handler/UI/accessibility tests, real-PostgreSQL concurrency/tenant/replay/expiry tests, and the real-SMTP browser sign/certificate journey cover the boundary. Closed-deal conversion continues in `1.3.10`; reusable terms/templates, approval, jurisdiction review, approved live-mailbox evidence, and pilot acceptance remain. FX disclosure is completed by `1.3.12`.
- `1.3.10` (signed quote to client handoff): complete locally; capability maturity remains foundation pending the remaining quote family outcomes and external validation. A writer—not the public signer—selects the actual same-tenant won stage and required close review for a native signed request while its deal remains open. One transaction locks the request/deal and binds certificate, immutable stage/reason snapshots, actor/time, activity/event, deal outcome, matching task rules, and existing client handoff. Digest-only idempotency collapses concurrent exact retries; changed reuse conflicts, another key cannot reuse terminal evidence, and an exact replay after a deliberate reopen returns the current deal without re-closing it. Cross-tenant database constraints bind stage, activity, and actor membership. Aggregate awaiting-conversion/converted metrics and an actionable alert, portable conversion evidence without replay hashes, handler/UI/accessibility coverage, disposable-PostgreSQL concurrency/tenant/reopen/export acceptance, and the real-SMTP browser journey through conversion and handoff cover the local outcome.
- `1.3.11` (active quote expiration and immutable reissue): complete locally. Every delivery now rejects an expired quote before persisting an intent or crossing a mailbox boundary, and the deal UI distinguishes active, expired, and replaced versions. A writer can reissue only an expired unsigned quote on an open deal with no unresolved send. One transaction serializes digest-only idempotency, locks the source/deal/signature state, creates a new PDF/version/SHA from the retained commercial content and a future validity date, binds one tenant/deal-constrained source/replacement lineage, voids an expired pending native request, and records activity/audit evidence without editing the original. Exact concurrent retry returns one replacement; changed reuse, repeat replacement, signed evidence, unresolved delivery, invalid lifecycle, and cross-tenant attempts fail closed. Portable export retains the lineage and original/replacement bytes but excludes replay hashes. Handler/role/UI/migration and disposable-PostgreSQL concurrency, tenant, evidence-retention, signing-state, and non-signature delivery-expiry tests cover the local outcome. Approved mailbox and pilot validation remain external evidence.
- `1.3.12` (immutable quote FX disclosure): complete locally. Every new finalization/reissue locks the workspace base currency and snapshots either an identity rate or the newest tenant/currency-pair rate effective on or before the document's UTC date, including source and rounded base-currency reporting total. Missing, future-only, malformed-source, and foreign-tenant rates fail before a quote or side effect exists and allow exact retry with the same digest-only idempotency key after configuration. Stored PDF, staff/customer views, audit metadata, and portable export share the disclosure while clearly retaining the quote-currency amount due. Later base/rate edits cannot rewrite history; an expired reissue deliberately uses the rate effective for its new date; legacy immutable versions remain labeled without an estimated backfill. Migration/handler/PDF/UI and disposable-PostgreSQL missing-rate, retry, effective-date, tenant, delivery, public-view, historical-retention, reissue, and audit tests cover the local outcome. Approved accounting-policy and pilot validation remain external evidence.
- `1.3.13` (revisioned quote preparation and independent approval): complete locally. Owners/admins manage tenant-scoped active or archived terms templates with bounded validity, delivery subject/message merge fields, signature defaults, optional required review, compare-and-swap revisions, unique active names, and audit history. Finalization accepts only an exact active template revision whose terms match, snapshots the template identity and rendered delivery defaults into the immutable quote, and requests approval when the quote, template, or workspace policy requires it. Enabling required review and finalizing recheck that a different active owner/admin exists. A different active owner/admin—not the requester or quote creator—can approve or reject the exact retained PDF digest with digest-only idempotency; rejection requires a note, terminal evidence cannot be changed, and pending/rejected versions are blocked both when delivery is prepared and immediately before the mailbox claim. Reissue retains the original template revision and requests fresh review. Notifications, activity/audit evidence, pending queue, deal UI, workspace portability without replay hashes, aggregate state/age metrics, stale alert/runbook, handler/unit/UI tests, freshly migrated PostgreSQL revision/self/cross-tenant/replay/claim/disabled-approver/export acceptance, and the two-admin real-PostgreSQL browser journey through blocked send, approval, SMTP delivery, signing, conversion, and WCAG checks cover the local outcome. Accounting policy, jurisdiction/consent review, approved live-mailbox evidence, and pilot validation remain external.
- `1.3.14` (bounded quote-template administration and complete active selection): complete locally. The member list API now defaults to 50, caps at 100 and offset 50,000, supports literal 100-character name search plus active/archived filters, and returns exact filtered totals with its stable active/name/ID page from one repeatable-read snapshot. Owners/admins see guarded previous/next navigation, mutation refresh, totals, and the explicit 100-active limit; stale loads cannot replace the current page and overlapping writes are suppressed. Create, update, archive, and policy writes revalidate and lock the active owner/admin, then use a tenant-namespaced transaction lock so two final-slot activations yield exactly one success. Deal preparation separately loads only active 100-row pages, deduplicates IDs, rejects mid-load total drift, and preserves access to legacy workspaces above the new ceiling. Handler/unit/UI tests, a freshly migrated 1,001-row PostgreSQL index/performance/literal-search/role/tenant/concurrency/recovery acceptance, the 38-row isolation guard, and the clean Chromium row-51/search/create/active-select/approval/signature/WCAG journey cover the outcome. Pilot validation of language and the 100-active operating ceiling remains.
...
- `1.3.9` Immutable native signing ceremony, typed-name consent, recipient decisions, and retained audit certificate: production-equivalent path complete; jurisdiction/pilot validation and the broader quote workflow remain.
- `1.3.10` Deliberate signed-quote conversion, close review, automation, and client handoff: complete locally; external validation and the broader quote workflow remain.
- `1.3.11` Active expiration and immutable reissue: complete locally; approved mailbox and pilot validation remain.
- `1.3.12` Effective-dated immutable quote FX disclosure: complete locally; accounting-policy and pilot validation remain.
- `1.3.13` Revisioned terms/delivery templates and independent exact-PDF approval: complete locally; accounting/jurisdiction, approved mailbox, and pilot validation remain.
- `1.3.14` Bounded quote-template administration, serialized active ceiling, and complete active-only deal selection: complete locally; operating-limit and pilot validation remain.
...
- `1.4.1` (embeddable lead capture forms foundation): complete and hardened at the application boundary. Organization-scoped forms use generated public IDs and map standard or active typed contact custom fields into attributed CRM leads while preserving the submitted business payload. Admin UI and embeds derive text/number/date/boolean/select controls and select options from current definitions, require complete coverage of required fields before activation, and use exact form revisions for safe concurrent edits. Mapped definition updates and inactive-field archival transactionally advance the affected form revision; active mapped fields cannot be archived. Admin definition changes are audited without values. Every public write requires a 256-bit, digest-only, organization/form/revision-bound challenge that is usable after two seconds and expires after 30 minutes. Submission re-locks the current form/challenge, validates typed values, snapshots a value-free mapping contract plus consent text/time, creates the contact/activity/submission, and consumes the challenge transactionally. Exact retries return the original effect while stale definitions, changed requests, premature/expired tokens, cross-form, and cross-tenant attempts fail closed. New submissions store no raw client address or user agent. Rolling migration/backfill, portable evidence, handler/component, disposable-PostgreSQL concurrency/tenant tests, and the Chromium typed hosted-page capture/assignment plus WCAG journey cover the local result. Edge/WAF reputation controls, automatic scoring/routing, conversion analytics, custom domains/SEO, and pilot validation remain future slices; reversible spam review is completed by `1.4.9`.
...
- `1.4.4` (list segmentation and dynamic/saved audiences foundation): complete as a hidden foundation. Added organization-scoped lead audience definitions with reusable filters, dynamic member-count previews against contacts, authenticated APIs for listing/creating/updating/previewing audiences, and a development-only Settings > Audiences UI for saving source/campaign/status/email-availability segments. The complete catalog now discloses an exact 100-total ceiling; new creates serialize the final slot, writes revalidate an active owner/admin, unsupported or oversized filters fail closed, and definition plus current count commit atomically. Listing and its complete count set use one read-only repeatable-read snapshot under a shared five-second deadline. Freshly migrated PostgreSQL acceptance proves role/tenant denial, distinct tenant counts, concurrent final-slot behavior, update recovery at capacity, complete 100-row retention, and rollback when a post-insert count is blocked. Production navigation and bundles omit it until audience activation, member drill-down, exclusions, advanced boolean logic, scheduled snapshots, audience observability, browser acceptance, and pilot validation perform the promised job.
- `1.4.5` (marketing email campaigns with scheduling and per-campaign analytics foundation): complete as a hidden foundation. Added organization-scoped marketing email campaign definitions tied to active saved audiences, stored schedule/status metadata, captured audience recipient-count snapshots, persisted per-campaign analytics counters, exposed authenticated APIs for listing/creating/updating campaigns, and retained a development-only Settings > Email Campaigns UI. The complete catalog now discloses an exact 100-total ceiling; creates serialize the final slot, writes revalidate an active owner/admin, and bounded definitions commit only with a locked same-tenant active-audience recipient snapshot under a shared five-second deadline. User input cannot manufacture the provider-owned `sent` outcome. Freshly migrated PostgreSQL acceptance proves role/tenant/audience denial, distinct counts, concurrent final-slot behavior, update recovery at capacity, complete 100-row retention, and rollback when recipient counting is blocked. Production navigation/bundles omit it until bulk recipient expansion, mailbox delivery, unsubscribe enforcement at send time, open/click attribution into campaign counters, send approvals, campaign reports, browser/provider acceptance, and pilot validation are complete.
- `1.4.6` (drip/nurture campaigns built on the sequence engine foundation): complete as a hidden foundation. Added organization-scoped nurture campaign plans that bind active saved audiences to existing email sequences, validate active campaigns against currently approved active sequence revisions, snapshot eligible audience counts, expose authenticated APIs for listing/creating/updating nurture campaigns, and retain a development-only Settings > Nurture Campaigns UI. The complete catalog now discloses an exact 100-total ceiling; creates serialize the final slot, and five-second writes revalidate an active owner/admin, lock the same-tenant active audience and same-tenant sequence, compute eligibility through that transaction, and commit only after every dependency succeeds. Freshly migrated PostgreSQL acceptance proves role/tenant/audience/sequence/approval denial, distinct counts, concurrent final-slot behavior, update recovery at capacity, complete 100-row retention, and rollback when eligible-contact counting is blocked. Production navigation/bundles omit it until automatic audience enrollment, enrollment refresh scheduling, suppression-aware bulk launch approvals, per-nurture performance rollups, reply/exit rules, browser/provider acceptance, and pilot validation are complete.
- `1.4.7` (rule-based lead scoring and routing/assignment foundation): complete as a hidden foundation. Added organization-scoped scoring rules over contact status, source, UTM, title, email, phone, and email-domain signals; persisted contact score/grade/scored-at metadata; exposed admin APIs for scoring-rule management plus a contact evaluation endpoint; routes unassigned contacts only when evaluation is manually invoked; logged lead scoring activity; surfaced retained read-only score evidence in contact list/detail; and retains a development-only Settings > Lead Scoring UI plus a tested API hook. The complete catalog now discloses an exact 100-stored-rule capacity; new creates serialize the final slot, definition writes revalidate an active owner/admin and same-tenant assignee, and evaluation transactionally revalidates an active writer, tenant contact, definition set, and every routing target. Freshly migrated PostgreSQL acceptance proves role denial, the concurrent final slot, foreign rule/assignee denial, both cross-tenant contact directions, disabled-target hiding, and exact effects. Production navigation and bundles omit both management and manual evaluation/routing; the bundle gate rejects the action copy and client endpoint path until automatic form evaluation, bulk rescoring, simulation, SLA queues, round robin, scoring-specific observability, browser acceptance, and pilot validation are complete.
...
- `1.4.10` (bounded lead-form administration and complete dependent selection): complete locally. The member list API defaults to 50, caps at 100 and offset 50,000, filters only all/active/inactive state, and returns an exact total plus stable active/update/ID page from one read-only repeatable-read snapshot. Settings exposes guarded previous/next navigation and reconciles writes on page one; obsolete responses cannot replace newer filter/page work. Submission review and automation load every all-status page for retained labels and filters, while landing pages and widgets request active forms only. One shared dependency-free traversal contract now serves lead forms and the existing saved-view/product/quote/email catalogs, ID-deduplicates every page, rejects total drift and same-total overlap, and fails explicitly beyond the reviewed addressable boundary. Handler/library/route tests, freshly migrated 1,001-row PostgreSQL direct-bound/index/active/adjacent/repeat/submission-count/tenant acceptance below two seconds, and the Chromium 52-form row-51 plus populated WCAG A/AA journey cover the outcome without a new migration because migration 42's composite index already matches the query. The 444-line route is 20.64/6.55 KiB; the shared refactor keeps entry/largest lazy chunks at 179.00/58.06 and 55.16/15.74 KiB, with aggregate assets at 812.36/252.49 KiB under narrowly advanced 813/253 KiB ceilings. Edge reputation, conversion analytics, automatic scoring/routing, custom domains/SEO, and pilot validation remain.
- `1.4.11` (bounded revision-safe landing-page and website-widget administration): complete locally. Both member catalogs now default to 50, cap at 100 and offset 50,000, filter only all/active/inactive state, and return an exact total plus stable active/update/ID page from one read-only repeatable-read transaction. Their settings routes share one stale-response-safe controller, expose accessible previous/next continuation, serialize mutations, load active form dependencies once without replacing an in-progress draft, preserve inactive historical form selections, and reconcile every successful write on page one. The lead-form builder likewise merges a late required custom-field dependency into existing input; deterministic delayed-response tests cover both boundaries. Migration 127 adds expand-safe positive revisions; a migration-126 fixture proves historical and post-upgrade old-writer rows default to revision 1 and both positive-value constraints validate; create/update revalidates an active owner/admin and same-tenant form inside a serializable transaction, active surfaces reject inactive forms, stale revisions conflict, and value-free audit commits or rolls back with the definition. Handler/route tests cover strict input and exact revision bodies. Freshly migrated PostgreSQL acceptance seeds 1,001 rows per catalog, asserts both existing tenant/status/update indexes, budgets active and adjacent pages below two seconds, and proves direct bounds, stable repeats, tenant/role/form denial, revision conflicts, transactional audit, and forced-audit rollback. The clean Chromium journey creates both public surfaces, starts with 52 definitions each, reaches both rows 51, and scans the populated catalogs plus public widget for WCAG A/AA before completing public landing-page capture. A measured follow-up consolidates editor lifecycle, catalog/editor structure, item actions, and controlled fields into the 242-line shared boundary, leaving route-specific files at 117/125 lines and their chunks at 4.02/1.59 and 4.47/1.69 KiB raw/gzip. The complete Node 24 build is now 179.04/58.06 KiB entry, 55.16/15.74 KiB largest lazy chunk, and 816.13/254.51 KiB aggregate under the unchanged 817/255 KiB ceilings. These definition writes have no asynchronous or external effect: stale work reloads in the UI and transactional failures leave nothing for an operator to repair, so no new operations runbook path is introduced. Edge reputation, conversion analytics, automatic scoring/routing, custom domains/SEO, richer embeds, and pilot validation remain.

- `1.4.12` (production-capable bounded external lead capture): complete locally. The generated form embed now escapes every definition-owned HTML, attribute, and script value; hydrates source URL and UTM attribution from the customer page; performs challenge and form-urlencoded submission without credentials; reports preparation, retry, validation, and success inline without navigation; and safely obtains a fresh challenge after success. First-party hosted pages/widgets likewise omit credentials. Only the exact public challenge/submission routes allow wildcard-origin `POST`/`OPTIONS` with `Content-Type`, never allow credentials, and leave private credentialed `ALLOWED_ORIGINS` plus CSRF behavior unchanged. All 15 public/authenticated routes and the internal operational-stats service method now own a five-second deadline; route timeouts map to stable `504 LEAD_CAPTURE_QUERY_TIMEOUT`. Fresh-schema PostgreSQL lock acceptance proves a timed-out submission leaves no contact, activity, submission, or consumed challenge, that the exact same challenge then succeeds and replays idempotently, that a timed-out definition create leaves no definition/audit, and that form/page/widget catalogs recover. Handler and frontend tests cover the full route/error and generated-markup contract. The clean PostgreSQL Chromium journey serves the real generated form from a separate live loopback customer origin, proves exact `Origin` and no `Cookie`, submits typed and required custom fields inline at the unchanged customer URL, retains source and UTM attribution through review/contact evidence, and passes WCAG A/AA. The production build measures 179.11/58.08 KiB entry, 54.10/15.64 KiB largest lazy chunk, and 811.70/255.96 KiB aggregate raw/gzip under unchanged 817/256 KiB aggregate ceilings. This promotes the bounded form/page/widget capture family to `production-capable`; approved edge/WAF and real-pilot evidence remain before pilot validation, while conversion analytics, custom domains/SEO, automatic scoring/routing, and broader campaign execution remain separate Version 1.4 expansion outcomes.

Candidate slices:

- `1.4.1` Embeddable web forms with typed standard/custom-field mapping to CRM records: production-capable through `1.4.12`.
- `1.4.2` Hosted landing pages with form capture and basic templates: production-capable through `1.4.12`.
- `1.4.3` Lead source and UTM/campaign attribution tracking: foundation complete.
- `1.4.4` List segmentation and dynamic/saved audiences: hidden foundation complete.
...
- `1.4.7` Rule-based lead scoring and routing/assignment: hidden foundation complete.
- `1.4.8` Lead capture from chat/website widget (optional): production-capable through `1.4.12`.
- `1.4.9` Lead submission spam review and reversible recovery: complete locally.
- `1.4.10` Bounded stable lead-form management plus complete drift-checked review/automation/page/widget selection: complete locally.
- `1.4.11` Bounded stable revision-safe landing-page and website-widget management with transactional audit: complete locally.
- `1.4.12` Credential-free safe external embed, uniform operation deadlines, and forced-timeout recovery: complete locally; production-capable bounded capture.
...
Version `1.5.12` makes activation itself fail closed: only reviewed task
contracts can become newly active, service writers revalidate owner/admin and
references inside the transaction, and one serialized 50-active-workflow-action
...
after mutations.
Version `1.5.14` reconciles durable lead execution with run inspection: the
tenant/type/run-key join projects retry, dead-letter, attempt, and error state,
dead work no longer inflates active health, and admins can use the existing
audited queue replay after following the affected run into Operations.
Version `1.5.15` gives every action in the two reviewed task contracts its own
immutable ordered outcome. Captured labels, schedules, attempts, terminal
...
Version `1.5.18` adds one exact non-recursive teammate notification after a
reviewed task plan and establishes the causal contract for every future action
that can emit another event. Recipient caps and replay are atomic; exact parent
actions, ancestor re-entry, depth, skipped evidence, metrics, audit, and
operator inspection are enforced before broader mutation actions can run.
Version `1.5.19` promotes the first trigger-capable mutation action: one exact
active teammate can receive a deal after creation, stage change, or direct
owner edit. The assignment, ordinary notification, typed changed/no-op result,
activity, audit, and nested owner-change event share the source transaction;
ancestor/depth protection now also caps fan-out after 50 existing causal runs.
Version `1.5.20` promotes one exact provider-bound action without inventing a
second delivery engine: a deal-created or real-stage-change event can enroll
its current primary contact in one approved active sequence, using the current
active deal owner's connected mailbox. Enrollment, typed evidence, audit, and
the first durable sequence job share the source transaction; missing contact
email, sender mailbox, membership, or executable sequence blocks the original
deal write without a partial result, while an existing active/paused enrollment
is an explicit idempotent no-op.
...
- `1.5.18` (causal guards and bounded teammate notification): complete locally. Expand-safe migration `122_workflow_causality_notifications.sql` retains an optional exact same-tenant successful parent run/action, root-or-nested depth shape, and a 0–50 notification result while preserving historical and rolling old-writer inserts. The exact `deal_task_notify_plan_v1` contract appends one literal owner/admin/current-owner in-app notification after a reviewed 1–5-task plan; membership is resolved in the source transaction, capped at 50 active recipients, idempotent per run/action/recipient, and zero/overflow recipients roll back every task, run, notification, activity, and audit effect. It is deliberately non-recursive and mutually exclusive with the approval gate. The causal boundary requires every future trigger-capable action to name its exact successful parent; repeated ancestor automations and the ninth nested hop are retained as skipped action/run evidence and content-free audit events instead of executing. Run inspection exposes root/nested identity, loop reason, and delivery count; finite workflow notification and loop-prevention metrics plus a validated warning alert provide operator evidence. Unit/UI tests, a rolling old-writer/cross-tenant migration test, fresh PostgreSQL replay/atomicity/recipient-cap/foreign-cause/re-entry/depth/metric/audit acceptance, a 43-row isolation guard, and the clean Chromium author/create/run/notification/deal-deep-link journey with populated WCAG scans verify the local outcome. The 433-line route delegates its 345-line pure model and 67-line run component; the measured build is 178.87/57.99 KiB entry, 55.12/15.73 KiB largest lazy chunk, 40.13/11.17 KiB automation route, and 778.16/242.36 KiB aggregate raw/gzip. Only aggregate ceilings advance from 774/242 to 779/243 KiB. General field/owner mutation, provider, branch, and arbitrary scheduled action execution remains hidden; pilot validation still remains.
- `1.5.19` (transactional deal-owner assignment): complete locally. Expand-safe migration `123_workflow_deal_owner_assignment.sql` adds exact typed target/change result columns plus a same-tenant membership foreign key without breaking historical rows or rolling old writers. The exact `deal_assign_owner_v1` contract permits one immediate `assign_owner` action after deal creation, a real stage change, or a direct owner edit and accepts only one active same-tenant target. The source transaction locks the deal, updates its assignment generation only on change, preserves ordinary preference-aware assignment notifications, records a `workflow.owner_assigned` activity and typed action/run/audit evidence, and emits the nested owner-change event from the exact successful action. Same-owner execution is an explicit no-op; replay is harmless; inactive/foreign targets or downstream failure roll back the complete source event. Ancestor/depth guards now also have a 50-existing-run causal-tree fan-out ceiling whose next branch is retained and audited as skipped. Unit/UI tests, rolling migration compatibility and cross-tenant constraint acceptance, fresh PostgreSQL direct-change/nested-event/re-entry/replay/no-op/notification/rollback/activation/causal-cap evidence, a guarded 44-row tenant matrix, and the clean Chromium author/edit/assignment-notification/root/nested/WCAG journey verify the outcome. The 453-line route delegates its 393-line pure model and 68-line run component; every production workflow Go file remains below 500 lines. The measured build is 178.87/57.99 KiB entry, 55.12/15.74 KiB largest lazy chunk, 42.80/11.65 KiB automation route, and 781.77/243.34 KiB aggregate raw/gzip. Only aggregate ceilings advance from 779/243 to 782/244 KiB. General field mutation, provider actions, arbitrary schedules, and broader branches remain hidden; pilot validation remains.
- `1.5.20` (transactional deal-to-sequence enrollment): complete locally. Expand-safe migration `132_workflow_sequence_enrollment.sql` adds tenant/exact-enrollment uniqueness, typed sequence/enrollment/contact/created action evidence, strict success/non-success shape checks, and validated composite foreign keys without breaking historical outcomes or rolling old writers. The exact `deal_add_to_sequence_v1` contract permits one immediate enrollment after deal creation or a real stage change and keeps every legacy `add_to_sequence` definition inert. Activation locks and validates one same-tenant approved active current-revision sequence; execution revalidates the current unarchived primary contact and email, active deal owner, and usable SMTP credentials or provider-specific Gmail/Microsoft OAuth send scope in the source transaction. A read-only OAuth grant fails before enrollment. A new active enrollment and its first idempotent `email_sequence.send` job commit with the deal, typed run/action result, activity, and content-free audit, while replay or another active/paused enrollment records an explicit retained no-op without duplicate work. Paused/stale-approved sequences, missing email/mailbox/send scope, inactive owner, foreign dependencies, mismatched stored evidence, and downstream failure reject without a partial source mutation. Manual enrollment enforces the same readiness with stable actionable conflicts, and every active executable rule exposes safety-only deactivation so an unavailable reference can be recovered without re-submitting stale definition content. Unit/handler/UI tests, rolling migration old-writer and tenant/exact-tuple constraints, fresh PostgreSQL activation/execution/replay/no-op/paused/email/mailbox/read-only-scope/owner/foreign evidence, a guarded 52-row tenant matrix, and the clean Chromium authoring/SMTP-as-deal-owner/run/contact/two-enrollment/WCAG journey verify the local outcome. The 473-line route delegates its 421-line pure model and 70-line run component; the new backend boundaries are 143 and 151 lines. The production Node 24 build is 179.16/58.10 KiB entry, 54.10/15.64 KiB largest lazy chunk, and 816.17/257.49 KiB aggregate raw/gzip under 190/65, 60/16, and 817/258 KiB ceilings. Only the aggregate-gzip ceiling advances from 257 to 258 KiB (about 0.4%); every entry, per-route, aggregate-raw, CSS, hidden-foundation, and source ceiling remains unchanged. General field mutation, arbitrary provider actions, schedules, and branches remain hidden; approved live-provider and pilot validation remain.

- `1.5.21` (transactional deal expected-close update): complete locally. Expand-safe migration `133_workflow_expected_close_date.sql` adds optional typed field/date/change evidence while preserving historical outcomes and rolling old writers, then validates the successful `update_field` result shape. The exact `deal_set_expected_close_v1` contract permits one immediate `update_field` action after deal creation or a real stage change, accepts only `expectedCloseDate` plus an integer 0–365-day UTC offset, and keeps every generic legacy `update_field` definition inert. Execution locks the unarchived source deal, derives the date from the transaction's UTC event date, and commits the source mutation, typed previous/current/change action evidence, activity on a real change, run, and content-free audit together. Replay is harmless and an already-equal date is an explicit no-op. Foreign events cannot observe or execute the rule; unsupported shapes cannot activate; and a forced downstream audit failure rolls back the date and every workflow effect. Unit/UI tests, rolling migration compatibility and strict evidence constraints, strict client response validation, fresh PostgreSQL execution/replay/no-op/tenant/legacy/rollback evidence, a guarded 53-row tenant matrix, and the clean Chromium author/create/date/run/WCAG journey verify the local outcome. The 479-line route delegates its 451-line pure model and 71-line run component; the new backend execution boundary is 125 lines and every production workflow Go file remains below 500 lines. The production Node 24 build is 179.16/58.10 KiB entry, 54.10/15.64 KiB largest lazy chunk, and 818.43/258.12 KiB aggregate raw/gzip under 190/65, 60/16, and 819/259 KiB ceilings. The narrowly reviewed aggregate ceilings advance by 2 KiB raw and 1 KiB gzip; every entry, per-route, CSS, hidden-foundation, and source ceiling remains unchanged. Every other field mutation, arbitrary provider action, schedule, and branch remains hidden; pilot validation remains.
...
- `1.5.12` Transactionally authorized reviewed activation with one exact 50-active-workflow-action workspace budget and legacy recovery: complete locally; pilot validation remains.
...
- `1.5.18` Exact causal parent/re-entry/depth guards plus one bounded non-recursive in-app teammate notification after deal tasks: complete locally; pilot validation remains.
- `1.5.19` Exact transactional deal-owner assignment with typed no-op/change evidence, normal notifications, nested owner-change causality, and a causal-tree fan-out ceiling: complete locally; pilot validation remains.
- `1.5.20` Exact transactional deal-primary-contact sequence enrollment with sender readiness, durable first delivery, typed no-op/create evidence, and real-SMTP Chromium acceptance: complete locally; approved provider and pilot validation remain.
- `1.5.21` Exact transactional expected-close update from the UTC deal-event date with typed previous/current/no-op evidence and atomic rollback: complete locally; pilot validation remains.
...
- `1.6.2` (grouped bar reports): production-capable for one bounded chart outcome. Production accepts only exactly one allowlisted category plus a record count, numeric sum, or numeric average and no ignored row columns. Migration `105_custom_report_grouped_bar_contract.sql` leaves every historical metadata-only bar unmarked; only a deliberately saved `grouped_bar_v1` definition executes or appears in normal navigation. It executes and exports through the same tenant-bound, archived-row-excluding, five-second report engine; includes visualization type/contract in execution and audit evidence; rejects mismatched client responses; and pairs every visual bar set with its exact paged accessible data table. Unit/handler/UI, real-PostgreSQL migration upgrade coverage for historical and rolling-old-app bars, freshly migrated PostgreSQL execution/export/legacy/foreign-tenant, 10,000-row performance, and two-workspace Chromium/WCAG evidence cover the outcome. Line, funnel, pie, and KPI metadata remain hidden foundations; personal dashboards, external sharing, chart/PDF export, and broader read models remain incomplete. Pilot validation remains external follow-up work.
- `1.6.3` (shared grouped-bar dashboard): production-capable for one bounded workspace outcome. Migration `124_shared_report_dashboard.sql` adds one revisioned dashboard per tenant, at most six ordered distinct widgets, half/full widths, composite tenant foreign keys, and rolling-writer compatibility. Writers configure only active `grouped_bar_v1` reports from Reports; all members see the same dashboard snapshot. Exact revisions and a tenant writer lock serialize updates, active membership and definitions are revalidated in the transaction, semantic no-ops do not advance history, audit failure rolls back the change, and portable export includes the configuration. Results execute in one read-only repeatable-read transaction under the five-second report deadline, return at most 12 categories per widget plus `hasMore`, and share one generated time. Handler/unit/UI and fresh/rolling PostgreSQL tests cover role, stale/concurrent/no-op, tenant/corrupt/inactive definition, timeout/recovery, export, and six-widget/10,000-contact performance; the two-workspace Chromium journey configures, renders, verifies exact accessible data, passes WCAG A/AA, and rejects foreign attachment. Personal dashboards, external sharing, drag-only layouts, and other chart families remain incomplete and hidden. Pilot validation remains external follow-up work.
- `1.6.4` (pipeline cohort conversion and velocity): production-capable as a fixed, explainable funnel outcome rather than activating the unrelated stored custom-funnel metadata. Members select one current pipeline, an exact stable entry stage, inclusive UTC cohort creation dates, an optional owner saved on the creation event, and a separate inclusive as-of date. The report uses the durable deal-stage ledger to return current open/won/lost outcomes, exact per-stage reach/current/exit counts, event-time forward-or-won exits, and median elapsed 24-hour days to first reach, completed visit exit, and current win. Its response explains skipped stages, re-entry, moves to another pipeline, current stage labels/order versus retained event-time math, cohort maturity, and incomplete pre-ledger coverage. The observation span is capped at 366 days, execution at five seconds, and output at the configured 20-stage pipeline limit. Handler/unit/UI tests, disposable-PostgreSQL exact 0/5/10/15-day math, renamed-stage, retained-owner, empty and foreign-ID acceptance, a 500-deal 14.8 ms local gate under two seconds, and the clean two-workspace Chromium/WCAG journey cover the local outcome. Real-pilot validation remains.
- `1.6.5` (revenue, activity, and cohort analytics): production-capable locally for the bounded fixed-report outcome. Every future deal-stage event atomically retains deal value/currency plus workspace base currency and the effective FX rate/date/source; won revenue sums only real event-time transitions into won and never reads mutable current deal/rate data. Missing values and missing conversion rates are counted separately and excluded rather than estimated, while a reopened-and-rewon deal remains a second outcome. The existing inclusive UTC date and retained-owner filters now apply consistently to activity, reasons, stage movement, and revenue inside one read-only repeatable-read five-second snapshot. Independent revenue coverage warns about historical and rolling-old-writer gaps. Fresh/rolling migration, immutable mutation, constraint, timeout, tenant/foreign-owner, missing-input, handler/UI, partial-index plan, pilot-volume two-second budget, and clean Chromium/WCAG journey evidence cover the local outcome. The production build is 812.34/256.46 KiB aggregate raw/gzip under reviewed 817/257 KiB ceilings; only aggregate gzip advances by 1 KiB for the production revenue presentation. Pilot validation remains.
- `1.6.8` (bounded saved-report definition management): complete locally. The management API defaults to 50 definitions, caps page size at 100 and offset at 50,000, and returns the exact stored-definition total from the same repeatable-read transaction as the page. Active/update/ID ordering is total and backed by migration `113_custom_report_definition_paging.sql`. Reports exposes accessible continuation, deduplicates IDs, rejects stale responses, serializes writes, and returns to page one after create or update while retaining a successful row and actionable reload error if reconciliation fails. Handler/unit/UI tests, freshly migrated 1,001-row adjacent-page/index/repeat/direct-bound/tenant PostgreSQL acceptance, and Chromium first-50/row-51 continuation before production table/bar creation, execution, and export cover the outcome. This closes management cardinality without activating stored line/funnel/pie/KPI foundations; pilot validation remains.
- `1.6.6` (scheduled saved-report CSV delivery): production-capable locally for one bounded email-export outcome. Migration `125_scheduled_report_delivery.sql` adds at most one revisioned daily/weekly UTC schedule per saved report, 20 retained schedules per tenant, 1–10 active member recipients, composite tenant constraints, occurrence history, and independent recipient states. Transactional due discovery captures one exact recipient set, durable source-guarded job, and audit row, then advances missed cadence directly to the next future time; the worker generates one formula-safe CSV artifact through the existing five-second/10,000-row report engine, stores its digest for seven days, and reuses it for every recipient and deliberate recovery. Provider acceptance finalizes exact evidence; definite rejection retries only through the bounded job; ambiguous contact becomes terminally `uncertain` and never auto-retries. An owner/admin can confirm sent or explicitly accept duplicate risk before retrying the retained artifact. Active scheduled-definition writes require the same role; definition deactivation/non-executability atomically pauses delivery, cancels untouched work, and requires deliberate resume. Inactive recipients skip; Postmark carries validated exact base64 attachments and treats malformed accepted responses as ambiguous; fake delivery is allowed only in development/test. Aggregate availability/active/overdue/uncertain/failed metrics and four alerts, retention cleanup, safe workspace export, stable API/UI recovery, fresh/rolling migration, handler/provider/unit tests, freshly migrated PostgreSQL role/tenant/audit-failure/disabled-member/replay/retry/revision-race/definition-lifecycle/concurrent-final-slot/cleanup acceptance, and the clean Chromium/WCAG/cross-tenant journey cover the local outcome. The Node 24 build keeps scheduled delivery in a separate 9.00/3.12 KiB chunk, the Reports route at 51.50/12.38 KiB, and aggregate assets at 805.82/250.58 KiB under reviewed 806/251 KiB ceilings. Approved Postmark sandbox and pilot evidence, recipient/timing validation, and PDF delivery remain external or future work.
...
- `1.6.3` One revisioned shared grouped-bar dashboard with accessible button-based ordering: production-capable locally; personal dashboards, external sharing, and pilot validation remain.
- `1.6.4` Exact pipeline-entry cohort conversion and velocity report: production-capable locally; pilot validation remains.
- `1.6.5` Event-time base-currency revenue, activity, and cohort analytics with date-range and retained-owner filters: production-capable locally; pilot validation remains.
- `1.6.6` Daily/weekly saved-report CSV email with durable per-recipient recovery: production-capable locally; approved Postmark/pilot validation and PDF delivery remain.
- `1.6.7` Query performance and read-model strategy for analytics at scale.
- `1.6.8` Stable bounded saved-report definition management with exact totals and visible continuation: complete locally; pilot validation remains.
