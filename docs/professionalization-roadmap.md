...
- `1.1.2` (automatic generic IMAP sync worker): complete. Added due-account discovery for enabled generic IMAP/password mailboxes and an in-process mailbox sync worker that starts after API boot, polls due accounts every 15 minutes, runs the same ingestion path as `POST /api/me/email-sync/run`, and logs batch import/failure counts. OAuth provider fetchers and a durable external job runner remain future slices.
...
- `1.1.8` (sequence reply detection foundation): complete. Inbound synced email that matches a contact now completes that contact's active/paused sequence enrollments and clears `next_send_at`, so replies stop future sequence sends once the sequence send worker exists.
