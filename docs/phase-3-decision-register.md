...
| `P3-E3` | Approved Google Workspace and Microsoft 365 test mailboxes plus OAuth app credentials/consent | For each provider, connect, refresh, send, inspect Sent, sync an unrelated inbound message plus an exact reply, prove only the exact header/provider-thread-correlated reply exits its sequence, and do not expose token/mail content | Redacted provider request/message/thread IDs, RFC-header preservation result, granted scopes, refresh/sync/send metrics, exact release, and reconnect/revocation behavior. |
