...
| `P3-E3` | Approved Google Workspace and Microsoft 365 test mailboxes plus OAuth app credentials/consent | For each provider, connect, refresh, send, inspect Sent, sync inbound/reply, and confirm sequence reply exit without exposing token/mail content | Redacted provider request/message IDs, granted scopes, refresh/sync/send metrics, exact release, and reconnect/revocation behavior. |
