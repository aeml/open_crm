...
- Accepted deploys now bound eligible API-image history to five by default while always protecting the current/previous rollback pair. Cleanup only considers valid tags in the configured repository whose embedded revision exactly matches, records atomic complete/partial evidence, retains historical manifests, and never force-removes mismatched or operator-managed tags. An exact-SHA configuration redeploy preserves the usable earlier rollback target, with both paths covered by disposable recovery acceptance.
- Team access mutations now close the stale-session/audit gap: role and status changes revalidate the acting owner/admin inside their serializable tenant transaction, and a changed role commits exactly one previous/current-role audit event with the role itself. Forced audit failure rolls back the access change, unchanged retries create no history, and the portable workspace package plus browser-audited CSV retain the result.
