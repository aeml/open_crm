...
- Many Part II capabilities persist definitions and expose management UI but do not perform the promised job. Examples include real telephony/calendar providers, marketing delivery, broad workflow action/scheduling execution, report dashboards/sharing/scheduled delivery, and public booking. Workflow execution now has bounded deal-event and conditional lead-form task outcomes: deal rules atomically create an explicitly versioned 1–5-task playbook with independent due dates and can use one explicitly versioned typed event-time snapshot condition without activating legacy definitions, while lead tasks retain exactly one immutable whole-day creation schedule, separate due offset, and reversible operator spam quarantine/recovery before future qualification or routing. An exact `deal_approval_task_plan_v1` contract can pause a captured 1–5-task deal plan before any task is created; eligible owner, admin, or current record-owner reviewers receive a bounded queue and notifications, and an idempotent approve/reject decision either creates every captured task atomically or creates none. Immutable action snapshots, current actor/requester/ownership checks, cancellation on definition changes or requester deactivation, audit/activity/export evidence, approval-specific metrics and alerting, rolling and fresh PostgreSQL tests, and Chromium acceptance cover that bounded human gate. All supported paths retain immutable ordered per-action evidence, including captured labels, lifecycle state, attempts, schedules, terminal reasons, and tenant-validated task outputs. Lead inspection also reconciles the exact tenant-scoped durable queue state, so retryable/dead work is not mislabeled as perpetually running and an admin can invoke the existing audited replay from retained trigger evidence. Unsupported action, schedule, and branch families remain hidden. Saved table and explicitly versioned grouped numeric bar reports now share a bounded tenant-safe screen, exact accessible table, and audited CSV runtime; pre-contract bar metadata and line/funnel/pie/KPI charts remain hidden. Stripe and first-party quote signing have executable production-equivalent contract paths, but both remain below pilot-validated maturity and still have named completion work.
