...
- At audit time, the frontend built as one roughly 531 kB minified JavaScript chunk. Route-level lazy loading now keeps the entry at 177.99 KiB and CI enforces entry/lazy/total/CSS raw+gzip budgets. Focused outreach, presentation, company-people, and commercial-state modules reduce the contact, company, and deal parent routes from 2,038, 984, and 1,064 lines to 740, 827, and 785 under tightened 750-, 850-, and 800-line ceilings; record-scoped outreach and company-person state reject late responses from prior selection epochs, including leave-and-return navigation. The API composition root is now 369 lines, with all 205 explicit registrations split into sub-500-line platform, foundation, and core-CRM files and covered by package-wide inventory/write-policy guards; extracting shared handler helpers also leaves every production `internal/app` file below 500 lines. The remaining record-route exceptions are the active maintainability hotspots.
