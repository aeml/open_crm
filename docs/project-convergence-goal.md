...
- Many Part II capabilities persist definitions and expose management UI but do not perform the promised job. Examples include real telephony/calendar providers, marketing delivery, workflow trigger/action execution, report query execution, and public booking. Stripe and first-party quote signing now have executable production-equivalent contract paths, but both remain below pilot-validated maturity and still have named completion work.
...
- At audit time, the frontend built as one roughly 531 kB minified JavaScript chunk. Route-level lazy loading now keeps the entry within its enforced raw/gzip budget, and CI also enforces every lazy chunk, total assets, and CSS budgets. Focused outreach, lead-score, contact workspace/detail orchestration, shared record selection/work, company directory/people/workspace/detail orchestration, deal directory/detail/commercial/workspace, and task quick-action/directory/workspace/detail modules reduce the contact, company, deal, and task parent routes from 2,038, 1,364, 1,365, and 1,093 lines to 449, 458, 460, and 496. All production route files are now below the default 500-line ceiling with no explicit exception. The 168-line contact create/detail workspace composes forms, scoring, outreach, customer context, review scheduling, touchpoints, and record work while preserving the contact lazy boundary, and a 134-line contact-detail orchestrator owns selection-aware route, record, related-deal, and work loading. Contact, company, and deal selection abort obsolete loads and distinguish repeated A-to-B-to-A visits; the 178-line company-detail orchestrator owns route, record, related-deal, work, and seeded-create state without weakening those guards, while a tested 176-line company-directory hook owns bootstrap data, filters, loading, and request identity so an older bootstrap or search cannot replace the latest directory. A tested 231-line deal-directory hook similarly owns bootstrap/options, filters, loading, URL/history synchronization, loading, and request identity without repeating the full bootstrap after each filter change. A 128-line task-detail orchestrator owns direct-route hydration, visit identity, cached snapshots, and selection-aware form state. Record-scoped outreach, lead-score, company-person, work, quote/signature, shared follower, and task mutation state reject late cross-record responses. Task-form saves also bind to the exact task visit and cannot navigate after the task route unmounts. Guarded snapshot actions serialize per record, expose their shared pending state so incompatible deal-commercial controls wait for durable completion, suppress duplicate work actions, and validate returned record/work identities. The API composition root is now 380 lines, with all 231 explicit registrations split into sub-500-line platform, foundation, and core-CRM files and covered by package-wide inventory/write-policy guards; extracting shared handler helpers, invitation lifecycle delivery, and system-email feedback also leaves every production `internal/app` file below 500 lines.
...
- At audit time, there were no browser-level end-to-end smoke tests. CI now runs a Chromium pilot journey against disposable PostgreSQL 16 plus an automated WCAG A/AA scan of login, signup, dashboard, core record, reporting, team, import, billing, and export surfaces; the larger Phase 2–4 outcomes still need to extend those journeys as they become executable.
