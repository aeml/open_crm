...
- At audit time, the frontend built as one roughly 531 kB minified JavaScript chunk. Route-level lazy loading now keeps the entry at about 175.5 KiB and CI enforces entry/lazy/total/CSS raw+gzip budgets, but the largest record routes and API composition/router remain maintainability hotspots.
