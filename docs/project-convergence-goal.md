...
- Many Part II capabilities persist definitions and expose management UI but do not perform the promised job. Examples include real telephony/calendar providers, marketing delivery, broad workflow action/scheduling/approval execution, report charts/dashboards/sharing/scheduled delivery, and public booking. Workflow execution now has bounded deal-event and conditional lead-form task outcomes: deal tasks support one explicitly versioned typed event-time snapshot condition without activating legacy definitions, while lead tasks retain an immutable whole-day creation schedule, separate due offset, and reversible operator spam quarantine/recovery before future qualification or routing. Unsupported families remain hidden. Saved table reports now execute through a bounded tenant-safe screen and audited CSV runtime, while richer analytics remain hidden. Stripe and first-party quote signing have executable production-equivalent contract paths, but both remain below pilot-validated maturity and still have named completion work.
