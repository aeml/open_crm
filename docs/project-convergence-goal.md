...
- Many Part II capabilities persist definitions and expose management UI but do not perform the promised job. Examples include real telephony/calendar providers, marketing delivery, broad workflow action/scheduling/approval execution, report dashboards/sharing/scheduled delivery, and public booking. Workflow execution now has bounded deal-event and conditional lead-form task outcomes: deal tasks support one explicitly versioned typed event-time snapshot condition without activating legacy definitions, while lead tasks retain an immutable whole-day creation schedule, separate due offset, and reversible operator spam quarantine/recovery before future qualification or routing. Unsupported families remain hidden. Saved table and explicitly versioned grouped numeric bar reports now share a bounded tenant-safe screen, exact accessible table, and audited CSV runtime; pre-contract bar metadata and line/funnel/pie/KPI charts remain hidden. Stripe and first-party quote signing have executable production-equivalent contract paths, but both remain below pilot-validated maturity and still have named completion work.
...
- At audit time, the frontend built as one roughly 531 kB minified JavaScript chunk. Route-level lazy loading now keeps the entry, every lazy chunk, aggregate assets, and CSS within CI-gated raw/gzip budgets. Focused contact/company/deal/task orchestration reduces their parent routes from 2,038, 1,364, 1,365, and 1,093 lines to 449, 458, 473, and 496 with no production-route exception to the 500-line ceiling. Selection-aware loading and guarded snapshot mutations reject obsolete cross-record responses, serialize incompatible commercial actions, suppress duplicate work, and validate returned identities. Production now includes the exact client-period activity report plus bounded saved-table and explicitly versioned grouped-bar builder/result/export paths while rejecting pre-contract bars and incomplete line/funnel/pie/KPI, dashboard, audience/scoring, booking, calling/SMS, and marketing/nurture management surfaces. The current measured build is 178.92/58.01 KiB entry, 54.78/15.64 KiB largest lazy chunk, and 698.98/219.95 KiB aggregate raw/gzip; the Reports route is 39.90/9.55 KiB, with a separate 162-line client-period component, 295-line saved-report orchestration, 245-line pure tested catalog/form model, and focused 26/33-line table/bar renderers. The task-automation route remains 436 lines after adding typed event-time deal conditions. The 409-line request composition file and all production `internal/app` files remain below 500 lines; all 247 explicit route registrations are split across sub-500-line registration files and covered by package-wide inventory and hosted-write-policy guards.
