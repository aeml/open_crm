...
- At audit time, the frontend built as one roughly 531 kB minified JavaScript chunk. Route-level lazy loading now keeps the entry, every lazy chunk, aggregate assets, and CSS within CI-gated raw/gzip budgets. Focused contact/company/deal/task orchestration reduces their parent routes from 2,038, 1,364, 1,365, and 1,093 lines to 449, 458, 473, and 496 with no production-route exception to the 500-line ceiling. Selection-aware loading and guarded snapshot mutations reject obsolete cross-record responses, serialize incompatible commercial actions, suppress duplicate work, and validate returned identities. Production now includes the bounded saved-table and explicitly versioned grouped-bar builder/result/export path while rejecting pre-contract bars and incomplete line/funnel/pie/KPI, dashboard, audience/scoring, booking, calling/SMS, and marketing/nurture management surfaces. The current measured build is 178.92/58.03 KiB entry, 54.78/15.64 KiB largest lazy chunk, and 692.40/219.14 KiB aggregate raw/gzip; the saved-report route is 33.91/8.81 KiB while its orchestration is 293 lines, its pure tested catalog/form model is 245 lines, and its focused table/bar renderers remain 26/33 lines. The task-automation route remains 436 lines after adding typed event-time deal conditions. The 408-line request composition file and all production `internal/app` files remain below 500 lines; all 246 explicit route registrations are split across sub-500-line registration files and covered by package-wide inventory and hosted-write-policy guards.
