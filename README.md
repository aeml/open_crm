...
- Tenant-scoped deal line items and totals, immutable versioned quote PDFs, durable connected-mailbox delivery with uncertain-send recovery, expiring customer access, approximate view/download evidence, explicit non-acceptance receipt, and an optional quote-bound typed-name consent ceremony with idempotent recipient decisions, staff void controls, and retained audit certificates
...
- Quote approval, reusable terms, automatic signed-quote close conversion, jurisdiction policy, and pilot/live-mailbox validation remain; the native immutable quote signing ceremony and certificate path are executable but are still labeled foundation until those outcomes pass. Marketing, general-purpose workflow, and report-builder foundations still need their real runtime outcome. Calling, SMS, calendar/booking-link, marketing-email, and nurture-campaign management foundations remain development-only until their promised end-to-end jobs exist
