...
- System identity email has a real Postmark adapter plus an authenticated, attempt-bound, idempotent bounce/complaint callback with complaint suppression, admin delivery guidance, secret-safe retention, and aggregate alerts; approved live-sandbox evidence is still pending. User mailbox email has real SMTP, IMAP, Gmail, and Microsoft Graph adapters, and sequence delivery uses the durable PostgreSQL runner, while customer-mail reply/bounce analytics and production deliverability controls remain incomplete
