...
test('pilot lead-to-client journey persists data and isolates tenants', async ({ browser, page }) => {
  ...
  await expect(page.getByRole('heading', { name: leadFollowUpRuleName })).toBeVisible()
	await expect(page.getByText('1 of 50 active workflow actions allocated. Each task, approval gate, and teammate notification uses one slot.')).toBeVisible()
  ...
  await expect(page.getByText(/2-task playbook/)).toBeVisible()
	await expect(page.getByText('4 of 50 active workflow actions allocated. Each task, approval gate, and teammate notification uses one slot.')).toBeVisible()
  ...
  await page.getByRole('button', { name: 'Create task rule' }).click()
  const notificationDefinition = page.getByRole('list', { name: 'Task automation rules' }).getByRole('listitem').filter({ hasText: dealNotificationRuleName })
  ...
  await expect(notificationDefinition).toContainText(`Notification: Workspace owners · “${dealNotificationMessage}”`)
	await expect(page.getByText('6 of 50 active workflow actions allocated. Each task, approval gate, and teammate notification uses one slot.')).toBeVisible()
  ...
  await page.getByRole('link', { name: 'Automations', exact: true }).click()
  const notificationAutomationRun = page.getByRole('list', { name: 'Task automation runs' }).getByRole('listitem').filter({ hasText: dealNotificationRuleName })
  ...
  const dealAutomationRun = page.getByRole('list', { name: 'Task automation runs' }).getByRole('listitem').filter({ hasText: dealApprovalRuleName })
