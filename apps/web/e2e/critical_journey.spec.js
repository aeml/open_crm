...
test('pilot lead-to-client journey persists data and isolates tenants', async ({ browser, page }) => {
  ...
  await expect(page.getByRole('heading', { name: leadFollowUpRuleName })).toBeVisible()
	await expect(page.getByText('1 of 50 active workflow actions allocated. Each task and approval gate uses one slot.')).toBeVisible()
  ...
  await expect(page.getByText(/2-task playbook/)).toBeVisible()
	await expect(page.getByText('4 of 50 active workflow actions allocated. Each task and approval gate uses one slot.')).toBeVisible()
