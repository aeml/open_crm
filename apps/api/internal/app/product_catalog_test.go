...
type fakeProductCatalogService struct {
	listResult      []moduleproductcatalog.Item
...
func (f *fakeProductCatalogService) ListByOrganization(_ context.Context, organizationID int64) ([]moduleproductcatalog.Item, error) {
...
func (f *fakeProductCatalogService) Archive(_ context.Context, organizationID, itemID int64) error {
...
func TestListProductCatalogItemsScopesToOrganization(t *testing.T) {
	service := &fakeProductCatalogService{
		listResult: []moduleproductcatalog.Item{{ID: 3, Name: "Implementation", SKU: "SERV-001", ItemType: "service", UnitPrice: "150.00", Currency: "USD", UnitName: "hour", IsActive: true}},
	}
	server := authenticatedProductCatalogServer(service, "member")

	request := httptest.NewRequest(http.MethodGet, "/api/product-catalog-items", nil)
	...
	if len(response.Data.Items) != 1 || response.Data.Items[0].SKU != "SERV-001" {
		t.Fatalf("unexpected catalog payload: %#v", response.Data.Items)
	}
}
...
func TestArchiveProductCatalogItemScopesToOrganization(t *testing.T) {
	...
	if service.lastArchiveOrg != 42 || service.lastArchiveID != 11 {
		t.Fatalf("unexpected archive routing: org=%d id=%d", service.lastArchiveOrg, service.lastArchiveID)
	}
}
