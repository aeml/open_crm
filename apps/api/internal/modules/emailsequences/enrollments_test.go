...
func TestSelectDueSendsSQLScopesAutomaticSending(t *testing.T) {
	...
	for _, expected := range []string{"seq.status = 'active'", "e.status = 'active'", "e.next_send_at <= now()", "e.enrolled_by_user_id is not null", "contact.archived_at is null", "coalesce(contact.email, '') <> ''"} {
