...
func (s *Service) Activity(ctx context.Context, organizationID int64, query Query) (Report, error) {
	...
	if query.OwnerUserID > 0 {
		var exists bool
		if err := s.pool.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM organization_memberships WHERE organization_id=$1 AND user_id=$2)`, organizationID, query.OwnerUserID).Scan(&exists); err != nil {
			return Report{}, fmt.Errorf("validate sales report owner: %w", err)
		}
		if !exists {
	...
	`, organizationID).Scan(&coverageStartedAt, &closeReasonCoverageStartedAt, &organizationCreatedAt); err != nil {
		...
		return Report{}, fmt.Errorf("load sales reporting coverage: %w", err)
	}
	report := Report{
	...
	owners, err := s.loadOwners(ctx, organizationID, from, endExclusive, query.OwnerUserID)
	if err != nil {
		return Report{}, err
	}
	report.Owners = owners
	stages, err := s.loadStages(ctx, organizationID, from, endExclusive, query.OwnerUserID)
	if err != nil {
		return Report{}, err
	}
...
func (s *Service) loadOwners(ctx context.Context, organizationID int64, from, to time.Time, ownerUserID int64) ([]OwnerSummary, error) {
	rows, err := s.pool.Query(ctx, `
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.UserID, &item.UserName, &item.Email, &item.Status, &item.DealsCreated, &item.StageMoves, &item.DealsWon, &item.DealsLost, &item.NotesAdded, &item.TasksCreated, &item.TasksCompleted); err != nil {
...
func (s *Service) loadStages(ctx context.Context, organizationID int64, from, to time.Time, ownerUserID int64) ([]StageConversion, error) {
	rows, err := s.pool.Query(ctx, `
		WITH metrics AS (
...
func (s *Service) loadEvents(ctx context.Context, organizationID int64, from, to time.Time, ownerUserID int64) ([]DealEvent, error) {
	rows, err := s.pool.Query(ctx, `
