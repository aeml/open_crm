...
func TestMetricsHandlerRendersBoundedRuntimeAndProcessMetrics(t *testing.T) {
	...
	collector.ObserveRecordEmailDeliveryRecovery("success", 4)
	collector.ObserveRecordEmailDeliveryRecovery("unexpected-outcome", -1)
	...
	handler := collector.Handler("monitoring-token-that-is-at-least-32", func(context.Context) RuntimeSnapshot {
		return RuntimeSnapshot{
			...
			NotificationEvents24h: map[string]int64{
				"deal.assigned":                2,
				"workflow.custom_notification": 6,
				"other":                        3,
			...
			RecordEmailDeliveriesAvailable:    true,
			RecordEmailDeliveriesSending:      5,
			RecordEmailDeliveriesStaleSending: 2,
			RecordEmailDeliveriesUncertain:    6,
			...
			WorkflowRunsAvailable:             true,
			WorkflowRunsQueued:                2,
			WorkflowRunsRunning:               3,
			WorkflowRunsFailed24h:             4,
			WorkflowRunsSkipped24h:            5,
			WorkflowLoopsPrevented24h:         7,
			WorkflowOldestActiveAge:           20 * time.Minute,
			WorkflowApprovalsPending:          6,
			WorkflowOldestApprovalAge:         26 * time.Hour,
			ScheduledReportsAvailable:         true,
			ScheduledReportActiveSchedules:    7,
			ScheduledReportActiveRuns:         3,
			ScheduledReportUncertain:          2,
			ScheduledReportFailed24h:          4,
			ScheduledReportOldestOverdueAge:   11 * time.Minute,
			LeadReviewsAvailable:              true,
			LeadReviewsUnreviewed:             6,
			LeadReviewsLegitimate:             7,
			LeadReviewsSpam:                   8,
			LeadReviewOldestUnreviewedAge:     25 * time.Hour,
			Backup: BackupStatus{
				Available:                   true,
	...
	for _, expected := range []string{
		...
		`open_crm_notification_events_24h{event_type="workflow.custom_notification"} 6`,
		...
		`open_crm_record_email_deliveries_available 1`,
		`open_crm_record_email_delivery_sending 5`,
		`open_crm_record_email_delivery_stale_sending 2`,
		`open_crm_record_email_delivery_uncertain 6`,
		`open_crm_record_email_delivery_recovery_runs_total{outcome="success"} 1`,
		`open_crm_record_email_delivery_recovery_runs_total{outcome="error"} 1`,
		`open_crm_record_email_delivery_recovered_total 4`,
		`open_crm_record_email_delivery_recovery_last_run_success 0`,
		...
		`open_crm_workflow_runs_available 1`,
		`open_crm_workflow_runs{status="queued"} 2`,
		`open_crm_workflow_runs{status="running"} 3`,
		`open_crm_workflow_runs{status="waiting_approval"} 6`,
		`open_crm_workflow_runs_failed_24h 4`,
		`open_crm_workflow_runs_skipped_24h 5`,
		`open_crm_workflow_loops_prevented_24h 7`,
		`open_crm_workflow_oldest_active_age_seconds 1200`,
		`open_crm_workflow_oldest_approval_age_seconds 93600`,
		`open_crm_scheduled_reports_available 1`,
		`open_crm_scheduled_report_active_schedules 7`,
		`open_crm_scheduled_report_active_runs 3`,
		`open_crm_scheduled_report_uncertain_recipients 2`,
		`open_crm_scheduled_report_failed_recipients_24h 4`,
		`open_crm_scheduled_report_oldest_overdue_seconds 660`,
		`open_crm_lead_reviews_available 1`,
		`open_crm_lead_reviews{state="unreviewed"} 6`,
		`open_crm_lead_reviews{state="legitimate"} 7`,
		`open_crm_lead_reviews{state="spam"} 8`,
		`open_crm_lead_review_oldest_unreviewed_age_seconds 90000`,
